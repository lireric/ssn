...
uint8* EXTERN_DNS_SERVERIP  = (uint8*)ETHERNET_DNS_SERVER;
...
uint8 dns_query(uint8 s, uint8 * name)
{
  ...
  switch(getSn_SR(s))
  {
    case SOCK_UDP:
      ...
      {
//        Delay_ms(1000);
        delay_nus2(1000*1000);
        dns_wait_time++;
        //if(ConfigMsg.debug) xprintfMsg("dns wait time=%d\r\n", dns_wait_time);
      }
    ...
    case SOCK_CLOSED:
      ...
      //if(ConfigMsg.debug) xprintfMsg("dns socket init over\r\n");
      len = dns_makequery(0, name, BUFPUB, MAX_DNS_BUF_SIZE);
      //if(ConfigMsg.debug) xprintfMsg("dns make query\r\n");
