...
void send_DHCP_DISCOVER(void)
{
  ...
  pRIPMSG = pvPortMalloc(sizeof(RIP_MSG));
...
void send_DHCP_REQUEST(void)
{
  ...
  //xprintfMsg("send dhcp request\r\n");
    sendto(SOCK_DHCP, (const uint8 *)pRIPMSG, sizeof(RIP_MSG), ip, DHCP_SERVER_PORT);
  //if(len==0)
    //xprintfMsg("send request error\r\n");
#ifdef DHCP_DEBUG
    xprintfMsg("sent DHCP_REQUEST\r\n");
#endif
}
...
void send_DHCP_RELEASE_DECLINE(char msgtype)
{
  ...
  if(msgtype)
  {
    ...
    pRIPMSG->OPT[i++] = GET_SIP[3];		
  #ifdef DHCP_DEBUG		
    xprintfMsg("sent DHCP_DECLINE\r\n");
  #endif		
  }
  #ifdef DHCP_DEBUG	
  else
  {
	  xprintfMsg("sent DHCP_RELEASE\r\n");
  }
  #endif	
...
uint8 parseDHCPMSG(uint16 length)
{
  ...
  {
    #ifdef DHCP_DEBUG
	  xprintfMsg("No My DHCP Message. This message is ignored.\r\n");
	  xprintfMsg("\tSRC_MAC_ADDR(%02X.%02X.%02X.",SRC_MAC_ADDR[0],SRC_MAC_ADDR[1],SRC_MAC_ADDR[2]);
	  xprintfMsg("%02X.%02X.%02X)",SRC_MAC_ADDR[3],SRC_MAC_ADDR[4],SRC_MAC_ADDR[5]);
	  xprintfMsg(", pRIPMSG->chaddr(%02X.%02X.%02X.",(char)pRIPMSG->chaddr[0],(char)pRIPMSG->chaddr[1],(char)pRIPMSG->chaddr[2]);
	  xprintfMsg("%02X.%02X.%02X)",pRIPMSG->chaddr[3],pRIPMSG->chaddr[4],pRIPMSG->chaddr[5]);
	  xprintfMsg("\tpRIPMSG->xid(%08X), DHCP_XID(%08X)",pRIPMSG->xid,(DHCP_XID));
	  xprintfMsg("\tpRIMPMSG->yiaddr:%d.%d.%d.%d\r\n",pRIPMSG->yiaddr[0],pRIPMSG->yiaddr[1],pRIPMSG->yiaddr[2],pRIPMSG->yiaddr[3]);
    #endif				
    return 0;
  }
  ...
  if( *((uint32*)DHCP_SIP) != 0x00000000 )
  {
    ...
    {
#ifdef DHCP_DEBUG		
    	xprintfMsg("Another DHCP sever send a response message. This is ignored.\r\n");
    	xprintfMsg("\tIP:%u.%u.%u.%u\r\n",svr_addr[0],svr_addr[1],svr_addr[2],svr_addr[3]);
#endif				
      return 0;								
    }
  }
  ...
  #ifdef DHCP_DEBUG	
  xprintfMsg("DHCP MSG received\r\n");
  xprintfMsg("yiaddr : %u.%u.%u.%u\r\n",GET_SIP[0],GET_SIP[1],GET_SIP[2],GET_SIP[3]);
  #endif	
  ...
  while ( p < e ) 
  {
    ...
    case dhcpMessageType :
      ...
      type = *p;
#ifdef DHCP_DEBUG			
      xprintfMsg("dhcpMessageType : %02X\r\n", type);
#endif			
...
#ifdef DHCP_DEBUG			
      xprintfMsg("dhcpIPaddrLeaseTime : %d\r\n", dhcp_lease_time);
#endif			
      break;
    case dhcpServerIdentifier :
...
#ifdef DHCP_DEBUG						
        xprintfMsg("My dhcpServerIdentifier : ");
        xprintfMsg("%u.%u.%u.%u\r\n", DHCP_SIP[0], DHCP_SIP[1], DHCP_SIP[2], DHCP_SIP[3]);
        xprintfMsg("My DHCP server real IP address : ");
        xprintfMsg("%u.%u.%u.%u\r\n", DHCP_REAL_SIP[0], DHCP_REAL_SIP[1], DHCP_REAL_SIP[2], DHCP_REAL_SIP[3]);
#endif						
      }
      else
      {
#ifdef DHCP_DEBUG			   
    	  xprintfMsg("Another dhcpServerIdentifier : \r\n");
    	  xprintfMsg("\tMY(%u.%u.%u.%u) ", DHCP_SIP[0], DHCP_SIP[1], DHCP_SIP[2], DHCP_SIP[3]);
    	  xprintfMsg("Another(%u.%u.%u.%u)\r\n", svr_addr[0], svr_addr[1], svr_addr[2], svr_addr[3]);
#endif				
      }
      break;
    default :
...
uint8 check_DHCP_state(SOCKET s) 
{
  ...
  else
  {
    if(dhcp_state == STATE_DHCP_READY)
    {
       //init_dhcp_client();
    #ifdef DHCP_DEBUG			
    	xprintfMsg("state : STATE_DHCP_READY\r\n");
    #endif	   	
    }
    ...
    {
    #ifdef DHCP_DEBUG	   
    	xprintfMsg("Fail to create the DHCPC_SOCK(%u)\r\n",SOCK_DHCP);
    #endif   
     return DHCP_RET_ERR;
    }
  ...
  switch ( dhcp_state )
  {
...
#ifdef DHCP_DEBUG			
        xprintfMsg("state : STATE_DHCP_REQUEST\r\n");
#endif			
      }
      else 
        check_DHCP_Timeout();
    ...
    case STATE_DHCP_REQUEST :
      if (type == DHCP_ACK) 
      {
        ...
        {
#ifdef DHCP_DEBUG					
        	xprintfMsg("state : STATE_DHCP_LEASED\r\n");
#endif		
        ...
        {
#ifdef DHCP_DEBUG					
        	xprintfMsg("state : STATE_DHCP_DISCOVER\r\n");
#endif				
          dhcp_state = STATE_DHCP_DISCOVER;
...
#ifdef DHCP_DEBUG
        xprintfMsg("state : STATE_DHCP_REREQUEST\r\n");
#endif
        reset_DHCP_time();
      }
      break;
    ...
    case STATE_DHCP_REREQUEST :
      if (type == DHCP_ACK) 
      {
        ...
        {
#ifdef DHCP_DEBUG
        	xprintfMsg("The IP address from the DHCP server is updated.\r\n");
        	xprintfMsg("OLD_SIP=%u.%u.%u.%u",OLD_SIP[0],OLD_SIP[1],OLD_SIP[2],OLD_SIP[3]);
        	xprintfMsg("GET_SIP=%u.%u.%u.%u\r\n",GET_SIP[0],GET_SIP[1],GET_SIP[2],GET_SIP[3]);
#endif
          return DHCP_RET_UPDATE;
        }
#ifdef DHCP_DEBUG
        ...
        {
        	xprintfMsg("state : STATE_DHCP_LEASED : same IP\r\n");
        }
#endif
        reset_DHCP_time();
        dhcp_state = STATE_DHCP_LEASED;
...
void check_DHCP_Timeout(void)
{
  if (dhcp_retry_count < MAX_DHCP_RETRY) 
  {
    if (dhcp_time > next_dhcp_time) 
    {
      ...
      case STATE_DHCP_DISCOVER :
  #ifdef DHCP_DEBUG			   
    	  xprintfMsg("<<timeout>> state : STATE_DHCP_DISCOVER\r\n");
  #endif				
        send_DHCP_DISCOVER();
        break;
      case STATE_DHCP_REQUEST :
  #ifdef DHCP_DEBUG			   
    	  xprintfMsg("<<timeout>> state : STATE_DHCP_REQUEST\r\n");
  #endif				
        send_DHCP_REQUEST();
        break;
      ...
      case STATE_DHCP_REREQUEST :
  #ifdef DHCP_DEBUG			   
    	  xprintfMsg("<<timeout>> state : STATE_DHCP_REREQUEST\r\n");
  #endif				
        send_DHCP_REQUEST();
        break;
      default :
...
uint8 check_leasedIP(void)
{
  ...
  #ifdef DHCP_DEBUG
	xprintfMsg("<Check the IP Conflict %d.%d.%d.%d: ",GET_SIP[0],GET_SIP[1],GET_SIP[2],GET_SIP[3]);
  #endif
