...
{
   uint16 idx = 0;
   if(len == 0) xprintfMsg("Unexpected2 length 0\r\n");
...
uint16 wiz_read_buf(uint32 addrbsb, uint8* buf,uint16 len)
{
  ...
  if(len == 0)
  {
	  xprintfMsg("Unexpected2 length 0\r\n");
  }
...
#ifdef __DEF_IINCHIP_DBG__
//          xprintfMsg("tx_size[%d]: %d, Sn_TXMEM_SIZE = %d\r\n",i, tx_size[i], IINCHIP_READ(Sn_TXMEM_SIZE(i)));
//          xprintfMsg("rx_size[%d]: %d, Sn_RXMEM_SIZE = %d\r\n",i, rx_size[i], IINCHIP_READ(Sn_RXMEM_SIZE(i)));
#endif
    SSIZE[i] = (int16)(0);
    RSIZE[i] = (int16)(0);
...
void send_data_processing(SOCKET s, uint8 *data, uint16 len)
{
  if(len == 0)
  {
	  xprintfMsg("CH: %d Unexpected1 length 0\r\n", s);
    return;
  }
...
void recv_data_processing(SOCKET s, uint8 *data, uint16 len)
{
  ...
  if(len == 0)
  {
	  xprintfMsg("CH: %d Unexpected2 length 0\r\n", s);
    return;
  }
