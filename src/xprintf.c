...
// ****************************************************************************************
void xputc2 (char** buff, char c)
{
	if (_CR_CRLF && c == '\n') xputc2(buff, '\r');		/* CR -> CRLF */
...
)
{
	...
	for (;;) {
		...
		case 'S' :					/* String */
			p = va_arg(arp, char*);
			for (j = 0; p[j]; j++) ;
			while (!(f & 2) && j++ < w) xputc2(&buff, ' ');
			xputs2(&buff, p);
			while (j++ < w) xputc2(&buff, ' ');
			continue;
		case 'C' :					/* Character */
			xputc2(&buff, (char)va_arg(arp, int)); continue;
		case 'B' :					/* Binary */
			r = 2; break;
		case 'O' :					/* Octal */
			r = 8; break;
		...
		if (f & 8) s[i++] = '-';
		j = i; d = (f & 1) ? '0' : ' ';
		while (!(f & 2) && j++ < w) xputc2(&buff, d);
		do xputc2(&buff, s[--i]); while(i);
		while (j++ < w) xputc2(&buff, ' ');
	}
...
void xsprintf2 (		/* Put a formatted string to the memory */
